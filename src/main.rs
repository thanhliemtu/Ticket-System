...
use axum::{Router, response::{Html, IntoResponse, Response}, routing::{get, post, get_service}, extract::{Query, Path}, middleware, Json, http::{Uri, Method}};
use ctx::Ctx;
...
async fn main() -> Result<()>{
    ...
    // Middleware only applies to the /api routes
...
async fn main_response_mapper(
    ...
    // Notice that the response's extension will only have the
    // Error type if we DO have an error.
